...
### Bombarda spell ###
execute as @s[tag=!usingStrengthPotion] at @s run summon snowball ^ ^ ^0.5 {damage:2d,Team:"NPC",NoGravity:1b,Color:-1,Tags:["bombardaSpell","bombarda_ini"],Passengers:[{id:"minecraft:armor_stand",Team:"NPC",Invulnerable:1b,Small:1b,Invisible:1b,Marker:1b,Tags:["spellEntity","bombardaDirection","bombardaDirection_ini"]}]}
execute as @s[tag=usingStrengthPotion] at @s run summon snowball ^ ^ ^0.5 {damage:3d,Team:"NPC",NoGravity:1b,Color:-1,Tags:["bombardaSpell","bombarda_ini","strengthAmplified"],Passengers:[{id:"minecraft:armor_stand",Team:"NPC",Invulnerable:1b,Small:1b,Invisible:1b,Marker:1b,Tags:["spellEntity","bombardaDirection","bombardaDirection_ini"]}]}
...
execute at @s as @e[tag=bombardaDirection_ini,limit=1,sort=nearest] store result entity @s Owner[0] int 1 run data get entity @p[tag=activePlayer] UUID[0]
execute at @s as @e[tag=bombardaDirection_ini,limit=1,sort=nearest] store result entity @s Owner[1] int 1 run data get entity @p[tag=activePlayer] UUID[1]
execute at @s as @e[tag=bombardaDirection_ini,limit=1,sort=nearest] store result entity @s Owner[2] int 1 run data get entity @p[tag=activePlayer] UUID[2]
execute at @s as @e[tag=bombardaDirection_ini,limit=1,sort=nearest] store result entity @s Owner[3] int 1 run data get entity @p[tag=activePlayer] UUID[3]
