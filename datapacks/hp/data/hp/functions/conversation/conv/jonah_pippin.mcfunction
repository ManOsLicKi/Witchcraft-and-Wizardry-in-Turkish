...
##### Conversation Header ######
################################
execute if entity @s[tag=use] as 0-0-6-0-9 run data merge entity @s {CustomName:"{\"text\":\"Hoş geldiniz, J. Pippin'in İksirleri. Her türlü rahatsızlığa karşı iksirler. Mağazamla ilgilendiğinize sevindim. Sizin için ne yapabilirim?\",\"color\":\"white\",\"bold\":\"false\"}"}
...
#-------------------------------
execute as @s[tag=use] as @e[type=armor_stand,limit=1,scores={con=3}] run data merge entity @s {CustomName:"{\"text\":\"≥ Elveda\"}",Tags:["_____Normal_____","_____click_____","convOption","i"]}
...
##### Conversation Header ######
################################
execute if entity @s[tag=use] as 0-0-6-0-9 run data merge entity @s {CustomName:"{\"text\":\"Tamamlanmış iksirlerden oluşan küçük bir seçkiye sahibim, ama aslında çoğunlukla malzeme satıyorum. Eğer iksirleri kendiniz yapmakla ilgilenmiyorsanız, Londra'daki Diagon Yolu'ndaki Elik's Elixirs'i ziyaret etmelisiniz. Aradığınız şey onda olabilir. \\n\\n\"}"}
execute if entity @s[tag=use] run tag @s add longHeader
