...
execute as @s[tag=!cancelPensive] store success score @s tmp run title @s actionbar ["",{"text":"Wait for everyone to interact with the pensive (","color":"gold"},{"score":{"name":"@s","objective":"tmp2"},"color":"gold"},{"text":" / ","color":"gold"},{"score":{"name":"@s","objective":"tmp3"},"color":"gold"},{"text":") ","color":"gold"}]
